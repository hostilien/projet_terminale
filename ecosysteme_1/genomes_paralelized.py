...
def init_simulation(N_POPULATION):
...
def run_round(round_id, genomes, nets,  N_POPULATION, N_STEPS, register = False):
    def vision(pos):
        x, y = pos
        vision = []
    ...
    positions, map, energies, food = init_simulation(N_POPULATION)
    log_positions = []
    log_energies = []
    log_food = []
    ...
    fitnesses = [0.0 for _ in range(N_POPULATION)]
...
def simulation(genomes, config):
    ...
    with ProcessPoolExecutor() as executor:
        futures = [executor.submit(run_round, i, genomes, nets,  N_POPULATION, N_STEPS, register = False if (G not in gen_to_record and i== 0) else True) for i in range(N_ROUNDS)]
    for future in futures:
        result = future.result()
        fitnesses = result["fitnesses"]
        log_food = result["log_food"]

        if len(log_food)>=2:
            log_positions = result["log_positions"]
            log_energies = result["log_energies"]
        
        for genome_id, (_, genome) in enumerate(genomes):
            genome.fitness += fitnesses[genome_id]/N_ROUNDS
